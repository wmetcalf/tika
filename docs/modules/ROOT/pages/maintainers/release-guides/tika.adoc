...
[IMPORTANT]
====
*Every Docker step in this guide is upstream-only and does nothing in this fork.*

`.github/workflows/docker-release.yml` and `docker-snapshot.yml` have been removed here, so
the automatic publishing this guide describes -- on a GA tag push, and via `workflow_dispatch`
-- will not run, and no images reach Docker Hub. A maintainer following this runbook for a GA
release would otherwise watch the Actions tab for a workflow that never starts and conclude the
images shipped.

Docker publishing happens upstream at https://github.com/apache/tika[apache/tika]. See
xref:maintainers/release-guides/docker.adoc[Releasing Tika Docker Images], which carries the
same caveat.
====
...
NOTE: Upstream only. This fork has no Docker workflows, so a bare tag publishes
nothing here. Keep using `X.Y.Z-rcN` anyway -- the tag scheme matters to the rest
of the release, and this guide should stay usable upstream.
...
----
. Create the GA git tag from the winning RC and push it. *Upstream, this
  auto-triggers the Docker release workflow* (see
  xref:maintainers/release-guides/docker.adoc[Releasing Tika Docker Images]).
  *In this fork it triggers nothing* -- the workflow has been removed:
+
[source,bash]
----
...
[source,bash]
----
gh workflow run docker-release.yml -f tag=X.Y.Z-beta-N -f build_number=1   # upstream only: this workflow is removed from this fork
----
+
...
WARNING: Upstream only. In this fork that workflow does not exist, so nothing
appears in the Actions tab and no images are published. Do not read its absence
as a build still pending. See xref:maintainers/release-guides/docker.adoc[Releasing
...
per the Docker guide. Upstream only: there is no such workflow to dispatch in
this fork.
