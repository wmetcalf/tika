...
[IMPORTANT]
====
*This fork does not publish Docker images, and the workflows described below have
been removed from it.*

`.github/workflows/docker-release.yml` and `docker-snapshot.yml` were deleted because they
could not run here: they filtered on a `main` branch this fork does not have, were never
registered with Actions, and the repository has no `DOCKERHUB_USER` / `DOCKERHUB_TOKEN`
secrets, so the login step would fail regardless.

Docker publishing for Apache Tika happens *upstream*, at
https://github.com/apache/tika[apache/tika]. Follow this guide there, not here. Any
`gh workflow run docker-release.yml` command below will fail in this fork.

The Dockerfiles under `tika-server/docker-build/` and `tika-grpc/docker-build/` are still
present and are usable for building images locally.
====
...
* `.github/workflows/docker-release.yml` — the release publishing workflow (upstream only; removed from this fork)
* `.github/workflows/docker-snapshot.yml` — the snapshot publishing workflow (upstream only; removed from this fork)
